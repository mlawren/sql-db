...
=item load_schema($name) -> SQL::DB::Schema

$name + '::$driver' (where $driver is a DBD driver name) must be the
name of a class which when loaded creates a schema called $name +
'::$driver'. See L<sqldb-schema> for how to generate such a class.
...
retrieved from other lexical scopes using the "load_schema" function.
