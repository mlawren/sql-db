...
[ If your needs are not very complicated you might consider
L<SQL::DBx::Simple> instead. ]

SQL::DB is capable of generating aggregate expressions, joins, nested
selects, unions and database-side operator invocations. Transactional
support is provided via L<DBIx::Connector>. There is some minor cross
database support which transparently handles the binding of binary data
types, keeping the amount of data- and database-specific code you have
to write to a minimum. For more basic needs you might consider starting
with the L<SQL::DBx::Simple> extension.

Although rows can be retrieved from the database as simple objects,
SQL::DB does not attempt to be an Object-Relational-Mapper al-la
...
These objects can be used with the do() method for INSERT, UPDATE, and
DELETE queries.

=item do(@query) -> $count

Prepares (a INSERT, UPDATE or DELETE) @query using either prepare() or
prepare_cached() (depending on the "prepare_cached" attribute) and
...
(depending on the "prepare_cached" attribute) and execute()'s it.
Croaks on error.
...
=item sth( @query ) -> ($sth, $rv) -> $rv

Prepares @query using either prepare() or prepare_cached() (depending
on the "prepare_cached" attribute) and execute()'s it. Croaks on error.

In array context returns a L<DBI::st> statement handle and the result
of the execute() call. This is useful if you want to obtain the data
with one of the L<DBI> fetchrow_hashref(), fetchall_arrayref() etc
methods.

In scalar context simply returns the result of the execute() call.

This method is the basis for all other retrieval methods.
...
for the do(), fetch(), fetch1(), iter() and sth() methods.
...
=item sql_cast(@args) -> L<SQL::DB::Expr>

Maps to "CAST($arg1, $arg2, ...)".
