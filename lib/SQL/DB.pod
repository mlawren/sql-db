...
L<DBI> do() call which is typically the number of rows affected.

=item update($table, set => \%values, where => \%expr) -> Int

Update rows in the database. Return the result of the underlying L<DBI>
...
SQL::DB jumped from version 0.18 to 0.19.1 due to a complete rewrite
...
from myself. 0.19.1 will be the last release marked as development,
